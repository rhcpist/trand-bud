...
<article class="module width_full">
    <form method="post" action="<?=base_url();?>index.php/admin/edit/edit_product/<?=$pages_info['id'];?>" enctype="multipart/form-data" id="product_form">
			<header><h3 style="text-align: center;">Редактировать товар</h3></header>
				<div class="module_content">
						...
						<fieldset>
							<label>Характеристики товара</label>
                                                        <textarea rows="2" name="description"><?=$pages_info['description'];?></textarea>
                                                        <div class="error" style="width:200px;"><?=form_error('description');?></div>
						</fieldset>
                                                <fieldset style="width:48%; float:left; margin-right: 3%;">
                                                        <label>Цена товара</label>
                                                        <input type="text" name="price" value="<?=$pages_info['price'];?>"/>
                                                </fieldset>   
						<fieldset style="width:48%; float:left; margin-right: 3%;"> <!-- to make two field float next to one another, adjust values accordingly -->
							<label>Выбрать фото</label>
							<div class="fileform"><div class="selectbutton">Обзор</div>
                                                    ...
                                                    <div class="submit_link" style="float:none;">
                                                        <div style="width: 260px; display:inline;">
                                                            ...
                                                            <select name="category_name">
                                                                <option value=""><?=$cat_info['cat_name'];?></option>
                                                                <?php foreach ($categories as $item):?>
                                                                <option value="<?=$item['id']?>" <?=set_select('category_name', $item['id']);?>><?=$item['title'];?></option>
                                                                <?php endforeach;?>
                                                            </select>
                                                        </div>
                                                        <div style="float:right;"><input id='xui' type="checkbox" name="akcia" value="Wrap.png" <?php $bool = ($cat_info['akcia']?'checked':''); echo $bool;?> ><span style="font-size:14px; color:red; padding-left:10px;">Стикер "Акция"</span></div>
                                                    </div>
